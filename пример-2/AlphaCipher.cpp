...
AlphaCipher::AlphaCipher(int key)
{
    this->k=key;
}
wstring AlphaCipher::CoderAlphaCipher(AlphaCipher w, wstring& s)
{
    wstring code;
...
wstring AlphaCipher::AlphaCipher(AlphaCipher w, wstring& s)
{
    s=getValidOpenText(s);
    w=getValidKey(w.k,s);
    int h;
...
inline int AlphaCipher::getValidKey(const int k, const std::wstring & s)
{
    if (k<=0)
